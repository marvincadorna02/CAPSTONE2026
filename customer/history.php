...
(function() {
  ...
  return false;
      ...
      // Lifecycle grouping: paid/claimed count as "completed", no_show as "cancelled"
      const DONE_STATUSES   = ['completed', 'paid', 'claimed'];
      const CANCEL_STATUSES = ['cancelled', 'no_show'];
      ...
      function updateTabCounts() {
        const completed = allHistory.filter(b => DONE_STATUSES.includes(b.status)).length;
        const cancelled = allHistory.filter(b => CANCEL_STATUSES.includes(b.status)).length;
      ...
      function renderHistory() {
        ...
        const filtered = currentFilter === 'all'
          ...
          : currentFilter === 'completed'
            ? allHistory.filter(b => DONE_STATUSES.includes(b.status))
            : allHistory.filter(b => CANCEL_STATUSES.includes(b.status));
        ...
        grid.innerHTML = filtered.map((b, i) => {
          ...
          const isCompleted = DONE_STATUSES.includes(b.status);
          const grp         = isCompleted ? 'completed' : 'cancelled';
          ...
          return `
            <div class="history-card ${grp}" style="animation-delay:${i * 0.05}s" data-status="${esc(b.status)}" id="card-${b.id}">
              <div class="hc-header">
                ...
                <div class="hc-right">
                  <span class="status-badge status-${grp}">${b.status.charAt(0).toUpperCase()+b.status.slice(1).replace('_',' ')}</span>
                 ${isCompleted ? `
...
btn.title = data.favorited ? 'Remove from Favorites' : 'Add to Favorites';
  ...
  setTimeout(() => window.location.reload(), 500);
      ...
      const STATUS_BG = {
        ...
        paid:'linear-gradient(135deg,#3b82f6,#2563eb)',
        claimed:'linear-gradient(135deg,#3b82f6,#2563eb)',
        cancelled:'linear-gradient(135deg,#ef4444,#dc2626)',
        no_show:'linear-gradient(135deg,#ef4444,#dc2626)'
      };
      ...
      function openDetail(b) {
        ...
        const dgrp = DONE_STATUSES.includes(b.status) ? 'completed' : 'cancelled';
        document.getElementById('modalStatusBadge').className = `status-badge status-${dgrp}`;
        document.getElementById('modalStatusBadge').textContent = b.status.charAt(0).toUpperCase()+b.status.slice(1).replace('_',' ');
...
async function loadNotifications() {
  ...
  paid:         (shop, reply) => `Payment confirmed by <span>${shop}</span>. Ready for pickup! 💰`,
  claimed:      (shop, reply) => `You claimed your device from <span>${shop}</span>! 🎉`,
  no_show:      (shop, reply) => `<span>${shop}</span> marked your booking as no-show.`,
