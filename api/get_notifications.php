...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($input['mark_one'])) {
        ...
        if ($type === 'booking' && !empty($input['booking_id']) && !empty($input['status'])) {
            $bId = (int)$input['booking_id'];
            $st  = (string)$input['status'];
            // Ownership check: booking must belong to this customer
            $chk = $conn->prepare("SELECT id FROM bookings WHERE id = ? AND customer_id = ?");
            $chk->bind_param("ii", $bId, $userId);
            $chk->execute();
            if ($chk->get_result()->fetch_assoc()) {
                $ins = $conn->prepare("INSERT IGNORE INTO notification_reads (user_id, booking_id, status_seen) VALUES (?, ?, ?)");
            ...
            $chk = $conn->prepare("SELECT id FROM reviews WHERE id = ? AND customer_id = ?");
            $chk->bind_param("ii", $rId, $userId);
            $chk->execute();
            if ($chk->get_result()->fetch_assoc()) {
                $ins = $conn->prepare("INSERT IGNORE INTO review_reply_reads (user_id, review_id) VALUES (?, ?)");
        ...
        // (customer/messages.php?open=id) already marks those messages read.
